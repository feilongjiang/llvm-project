...
SDValue Cpu0TargetLowering::getTargetNode(BlockAddressSDNode *N, EVT Ty,
                                          SelectionDAG &DAG,
                                          unsigned Flag) const {
  return DAG.getTargetBlockAddress(N->getBlockAddress(), Ty, Flag);
}
...
Cpu0TargetLowering::Cpu0TargetLowering(const Cpu0TargetMachine &TM,
  ...
  // Without this, every float setcc comes with a AND/OR with the reult,
...
MachineBasicBlock *Cpu0TargetLowering::emitAtomicBinary(MachineInstr &MI,
  ...
  if (Nand) {
    ...
    //  nor storeval, $0, andres    ; storeval = ~(oldval & incr)
...
MachineBasicBlock *Cpu0TargetLowering::emitAtomicBinaryPartword(
  ...
  BuildMI(BB, DL, TII->get(Cpu0::SHLV), Mask)
      .addReg(MaskUpper)
      .addReg(ShiftAmt);
  BuildMI(BB, DL, TII->get(Cpu0::NOR), Mask2).addReg(Cpu0::ZERO).addReg(Mask);
  ...
  //   and     maskedoldval0,oldval,mask2   ; mask2 = ~mask
  //   or      storeval,maskedoldval0,newval
  //   sc      success,storeval,0(alignedaddr)
  //   beq     success,$0,loopMBB

  BB = loopMBB;
  BuildMI(BB, DL, TII->get(Cpu0::LL), OldVal).addReg(AlignedAddr).addImm(0);
  if (Nand) {
    //  and andres, oldval, incr2
    //  nor binopres, $0, andres     ; binopres = ~(oldval & incr2)
    //  and newval, binopres, mask
    BuildMI(BB, DL, TII->get(Cpu0::AND), AndRes).addReg(OldVal).addReg(Incr2);
    BuildMI(BB, DL, TII->get(Cpu0::NOR), BinOpRes)
        .addReg(Cpu0::ZERO)
        .addReg(AndRes);
  ...
  BuildMI(BB, DL, TII->get(Cpu0::OR), StoreVal)
      ...
      .addReg(NewVal);
  BuildMI(BB, DL, TII->get(Cpu0::SC), Success)
      .addReg(StoreVal)
      .addReg(AlignedAddr)
      .addImm(0);
...
MachineBasicBlock *Cpu0TargetLowering::emitAtomicCmpSwapPartword(
  ...
  //  loop2MBB:
  //    and     maskedoldval1,oldval,mask2   ; mask2 = ~mask, clears sub-word
  //    bits or      storeval,maskedoldval1,shiftednewval sc
  //    success,storeval,0(alignedaddr) beq     success,$0,loop1MBB
  BB = loop2MBB;
  BuildMI(BB, DL, TII->get(Cpu0::AND), MaskedOldVal1)
      .addReg(OldVal)
      .addReg(Mask2);
...
static bool CC_Cpu0S32(unsigned ValNo, MVT ValVT, MVT LocVT,
                       ...
                       CCState &State) {
  // ByVal args are handled separately by handleByValArg.
  if (ArgFlags.isByVal()) {
    return true;
  }
...
static bool CC_Cpu0O32(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  // is true: function is vararg, argument os 3rd or higher, there is previous
...
SDValue Cpu0TargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Analyze operands of the call, assign locaitons to each operand.
  ...
  // Get a count of how many bites are to be pushed on the stack.
  unsigned NextStackOffset = CCInfo.getNextStackOffset();

#ifdef ENABLE_GPRESTORE
  if (!Cpu0ReserveGP) {
    // If this is the first call, crete a stack frame object that points to
    // a locaiton to which .cprestore saves $gp.
  ...
  // Check if it's really possible to di a tail call.
  if (IsTailCall) {
  ...
  // transform all store nodes into single node because all store
  ...
  bool IsPICCall = IsPIC; // ture if calls are traanslated to jalr $t9
...
SDValue Cpu0TargetLowering::LowerFormalArguments(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (ABI.IsO32() && IsRegLoc) {
      ...
      // Transform the physical register into a virtual register.
      ...
      // Handle f32/f64 -> i32/i64 bitcast for softfloat
...
Cpu0TargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  // the return vsalue to a location
  SmallVector<CCValAssign, 16> RVLocs;
  MachineFunction &MF = DAG.getMachineFunction();
  ...
  for (unsigned i = 0; i != RVLocs.size(); ++i) {
    ...
    // Guarantee that all emitted copies ar stuck together with flags.
...
CCAssignFn *Cpu0TargetLowering::Cpu0CC::fixedArgFn() const {
  if (IsO32) {
    return CC_Cpu0O32;
  } else {
    return CC_Cpu0S32;
...
CCAssignFn *Cpu0TargetLowering::Cpu0CC::varArgFn() const {
  if (IsO32) {
    return CC_Cpu0O32;
  } else {
    return CC_Cpu0S32;
