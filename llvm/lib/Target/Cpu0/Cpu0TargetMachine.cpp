...
static std::string computeDataLayout(const Triple &TT, StringRef CPU,
  ...
  // Pinters are 32 bit on some ABIS.
  Ret += "-p:32:32";
  ...
  // 32 bit registers are always available and stack is at least 64 bit
  // aligned.
  Ret += "-n32-S64";
...
// On function prologue, the stack is crated by decrementing
// it pointer. Once decremented, all references are done with positive
...
Cpu0TargetMachine::getSubtargetImpl(const Function &F) const {
  ...
  if (!I) {
    ...
    // function that reside in the TargetOptions.
