...
enum NodeType : unsigned {
  ...
  // No relocation with Cpu0 Hi register
  Hi,

  // Get the Lower 16 bits from a 32-bit immediate
  // No relocation with Cpu0 Lo register
  Lo,
...
protected:
  ...
  // This method creates the following nodesm which are necessary for
  ...
  // This method creates the following nodes, which are necessary for
  // computing a global symbol's address in larget-GOT mode:
  //
  // (load (wrapper (add %hi(symn), $gp), %lo(sym)))
  template <class NodeTy>
...
protected:
  ...
  // This lcass provides methods used to analyze formal and call
  ...
  public:
    ...
    // The size of the area the caller reserves for
    ...
    // Return the number of integer argument registers
  ...
  private:
    ...
    // Returns true if the calling convention allows the use if registers
    // to pass byval arguments
    bool useRegsForByval() const { return CallConv != CallingConv::Fast; }

    // Return the function that analyzes variable argument list functions.
    CCAssignFn *fixedArgFn() const;
    ...
    /// Return true if the byval argument should use registers
...
private:
  ...
  // Copy argument registers whic were used to pass a byval gargument to
  // the stack. Create a stack frame object for the byval argument.
...
#endif // LLVM_LIB_TARGET_CPU0_CPU0ISELLOWERING_H
