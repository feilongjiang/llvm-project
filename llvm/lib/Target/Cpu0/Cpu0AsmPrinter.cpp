...
void Cpu0AsmPrinter::emitInstruction(const MachineInstr *MI) {
  ...
  do {
    // Do any auto-generated pesudo lowerings.
    ...
    // extracts MCInst form MachineInstr.
...
// registers. For CPU registers consider LR, GP and FP for saving of necessary.
void Cpu0AsmPrinter::printSavedRegsBitmask(raw_ostream &O) {
  // CPU and FPU Saved Registers Bitmask
...
void Cpu0AsmPrinter::emitFunctionBodyEnd() {
  // There are instruction for this macros, but the must
  // always be at the function end, and we can't emit and
  // break with BB logic.
  if (OutStreamer->hasRawTextSupport()) {
    if (Cpu0FI->getEmitNOAT()) {
      OutStreamer->emitRawText("\t.set\tat");
    }
...
void Cpu0AsmPrinter::emitStartOfAsmFile(Module &M) {
  ...
  // Tell the assembler which AIB we are using.
  if (OutStreamer->hasRawTextSupport()) {
...
bool llvm::Cpu0AsmPrinter::isLongBranchPseudo(int Opcode) const {
