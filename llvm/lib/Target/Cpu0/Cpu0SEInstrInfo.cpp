//===-- Cpu0SEInstrInfo.cpp - Cpu0 Instruction Information ----------------===//
//
...
unsigned Cpu0SEInstrInfo::loadImmediate(int64_t Imm, MachineBasicBlock &MBB,
  ...
  // The first instruction can be LUi, which is different from other
...
void Cpu0SEInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (Cpu0::CPURegsRegClass.contains(DestReg)) {
    // Copy to CPU Reg.
  ...
  } else if (Cpu0::CPURegsRegClass.contains(SrcReg)) {
    // Copy from CPU Reg.
    if (DestReg == Cpu0::HI) {
...
void Cpu0SEInstrInfo::storeRegToStack(
